...
import com.example.batch7.ch4.dto.req.ResetPasswordModel;
...
import com.example.batch7.ch4.utils.SimpleStringUtils;
...
public class ForgetPasswordController {
    ...
    public Map cheKTOkenValid(@RequestBody ResetPasswordModel model) {
        if (model.getOtp() == null) return templateCRUD.templateEror("Token is required");
    ...
    public Map resetPassword(@RequestBody ResetPasswordModel model) {
        if (model.getOtp() == null) return templateCRUD.templateEror("Token is required");
        if (model.getNewPassword() == null) return templateCRUD.templateEror("New Password is required");
        User user = userRepository.findOneByOTP(model.getOtp());
        String success;
        if (user == null) return templateCRUD.templateEror("Token not valid");
